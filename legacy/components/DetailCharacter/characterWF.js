...
export function CharacterWithFetch({ character }) {
  return character.map(({ name, id, thumbnail, description, comics, series, stories, events }) => {
    return (
      <article key={id} className="cardDetailCharacter">
        <Helmet>
          <title>{`${name} details`}</title>
          <meta name="description" content={`This page is the details of the search ${character.name}`} />
        </Helmet>
        <div className="wrpaImgDetail">
          <img className="imgDetailCharacter" src={`${thumbnail.path}.${thumbnail.extension}`} />
        </div>

...
export function useGetCharacter(keyword) {
  const { loading, page, characterContext, setCharacterContext, setLoading, setPage } = reducerCharacter();

  useEffect(
    function () {
      // if (Boolean(characterContext) && keyword) return
      setLoading(true);
      getCharacters({ keyword }).then((character) => {
  ...
  useEffect(
    function () {
      if (page === 0) return;
      getCharacters({ keyword, page }).then((nextCharacter) => {
